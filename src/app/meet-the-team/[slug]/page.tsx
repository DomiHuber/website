...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!member) {
    return {
      title: 'Team Member Not Found | Swiss Bitcoin Institute',
    };
  }
...
export async function generateStaticParams() {
  ...
  // Only return core team members, not fellows (they have their own route)
  return members
...
export default async function TeamMemberPage({ params }: PageProps) {
  ...
  // Redirect fellows to their own route
  if (member.category === 'fellow') {
    redirect(`/fellows/${params.slug}`);
  }
  ...
  const nextMember = currentIndex < allCoreMembers.length - 1 ? allCoreMembers[currentIndex + 1] : null;
  const prevMember = currentIndex > 0 ? allCoreMembers[currentIndex - 1] : null;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="swiss-section-sm bg-white border-b">
        <div className="swiss-grid">
          <Button variant="ghost" asChild>
            <Link href="/meet-the-team" className="flex items-center gap-2">
              ...
              <div className="flex-1 text-center md:text-left">
                <div className="mb-4">
                  <Badge variant="secondary" className="mb-4">
                    {member.category === 'core' ? 'Core Team' : 'Research Fellow'}
                  </Badge>
                </div>
      ...
      {/* Navigation to Next/Previous Team Member */}
              ...
              <div className="flex items-center justify-between">
                {prevMember ? (
                  <Button variant="outline" asChild className="flex items-center gap-2">
                    <Link href={`/meet-the-team/${prevMember.slug}`}>
                ...
                <Button variant="ghost" asChild>
                  <Link href="/meet-the-team">View All Team</Link>
                </Button>
                
                {nextMember ? (
                  <Button variant="outline" asChild className="flex items-center gap-2">
                    <Link href={`/meet-the-team/${nextMember.slug}`}>
          ...
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button variant="outline" size="lg" asChild className="bg-white text-gray-900 hover:bg-gray-100">
                <Link href="/meet-the-team">View Full Team</Link>
              </Button>
            </div>
          </div>
        </div>
